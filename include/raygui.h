...
//----------------------------------------------------------------------------------
    ...
    // : Texture2D type is very coupled to raylib, required by Font type
    ...
    // : Font type is very coupled to raylib, mostly required by GuiLoadStyle()
    // It should be redesigned to be provided by user
    typedef struct Font {
...
// : Which text styling properties should be global or per-control?
...
int GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content, Vector2 *scroll, Rectangle *view)
{
    ...
    if (horizontalScrollBar.width < RAYGUI_MIN_SCROLLBAR_WIDTH)
    {
        ...
        mouseWheelSpeed = 30.0f;    // : Calculate speed increment based on content.height vs bounds.height
    }
    if (verticalScrollBar.height < RAYGUI_MIN_SCROLLBAR_HEIGHT)
    {
        verticalScrollBar.height = RAYGUI_MIN_SCROLLBAR_HEIGHT;
        mouseWheelSpeed = 30.0f;    // : Calculate speed increment based on content.width vs bounds.width
    }
...
int GuiDropdownBox(Rectangle bounds, const char *text, int *active, bool editMode)
{
    ...
    *active = itemSelected;
    ...
    // : Use result to return more internal states: mouse-press out-of-bounds, mouse-press over selected-item...
...
int GuiTextBox(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    ...
    bool multiline = false;     // : Consider multiline text input
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    ...
    // : Implement methods to calculate cursor position properly
...
int GuiColorBarHue(Rectangle bounds, const char *text, float *hue)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        // : Use directly DrawRectangleGradientEx(bounds, color1, color2, color2, color1);
...
static Rectangle GetTextBounds(int control, Rectangle bounds)
{
    ...
    switch (control)
    {
        ...
        case DROPDOWNBOX:
        case LISTVIEW:
            // : Special cases (no label): COMBOBOX, DROPDOWNBOX, LISTVIEW
        case SLIDER:
        case CHECKBOX:
        case VALUEBOX:
        case SPINNER:
            // : More special cases (label on side): SLIDER, CHECKBOX, VALUEBOX, SPINNER
        default:
        {
...
static void GuiDrawText(const char *text, Rectangle textBounds, int alignment, Color tint)
{
    ...
    for (int i = 0; i < lineCount; i++)
    {
        ...
        for (int c = 0, codepointSize = 0; c < lineSize; c += codepointSize)
        {
            ...
            if (codepoint == 0x3f) codepointSize = 1;       // : Review not recognized codepoints size
            ...
            else if (wrapMode == TEXT_WRAP_WORD)
            {
                ...
                // : Consider case: (nextSpaceWidth >= textBounds.width)
            }

            if (codepoint == '\n') break;   // WARNING: Lines are already processed manually, no need to keep drawing after this codepoint
            else
            {
                // : There are multiple types of spaces in Unicode,
...
static const char **GuiTextSplit(const char *text, char delimiter, int *count, int *textRow)
{
    ...
    // : HACK: GuiTextSplit() - Review how textRows are returned to user
