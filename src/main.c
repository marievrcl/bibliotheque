...
char livreASupprimerBuf[10] = "", userASupprimerBuf[10] = "";

// Flags TextBox actifs
    ...
    while(!WindowShouldClose())
    {
        ...
        else if(mode==AJOUT_USER)
        {
            ...
            if(GuiButton((Rectangle){140,270,200,40},"Valider"))
            {
                ...
                {
                    DrawText("Veuillez remplir tous les champs !",140,320,20,RED);
                }
                else
                {
        ...
        else if(mode==EMPRUNTS)
        {
            ...
            DrawText("Cette fonctionnalité sera implémentée",50,80,20,LIGHTGRAY);
            if(GuiButton((Rectangle){600,500,250,40},"Retour menu")) mode=MENU;
        }
