...
static void clearInput(void) { int c; while ((c = getchar()) != '\n' && c != EOF) {} }

// --------- Utils dates (JJ/MM/AAAA) ----------
static int estBissextile(int y){ return (y%4==0 && y%100!=0) || (y%400==0); }
...
static int indexUserParId(Utilisateur *u, int nb, int id){
    for(int i=0;i<nb;i++) if(u[i].id==id) return i;
    return -1;
}
...
static int nextEmpruntId(Emprunt *e, int nb){
    int max=0; for(int i=0;i<nb;i++) if(e[i].idEmprunt>max) max=e[i].idEmprunt;
    return max+1;
}
...
{
    int c; // pour vider scanf restants
    printf("\n--- EMPRUNTER UN LIVRE ---\n");
    int idLivre, idUser;

    printf("ID du livre : ");
    if (scanf("%d",&idLivre)!=1){ clearInput(); printf("Entrée invalide.\n"); return; }
    clearInput();
    ...
    if (scanf("%d",&idUser)!=1){ clearInput(); printf("Entrée invalide.\n"); return; }
    clearInput();
...
void retournerLivre(Emprunt *emprunts, int *nbEmprunts, int idLivre)
{
    ...
    // le champ retard sera (re)posé par verifierRetards ou on peut le poser ici
    int d = diffJours(emprunts[idx].dateEmprunt, dateR);
    emprunts[idx].retard = (d > 15) ? 1 : 0;

    printf(" Livre %d rendu. %s\n", idLivre, emprunts[idx].retard ? "️ Retard détecté (>15j)" : "Retour dans les temps");
}
...
void verifierRetards(Emprunt *emprunts, int nbEmprunts)
{
    ...
    for (int i=0;i<nbEmprunts;i++){
        // ne considérer que les emprunts non rendus
        if (emprunts[i].dateRetour[0] == '\0' || strcmp(emprunts[i].dateRetour,"-")==0){
            ...
            if (d > 15){
                ...
                printf("• Emprunt #%d  Livre:%d  Utilisateur:%d  Jours:%d  -> RETARD\n",
                       emprunts[i].idEmprunt, emprunts[i].idLivre, emprunts[i].idUtilisateur, d);
            }
        }
    }
