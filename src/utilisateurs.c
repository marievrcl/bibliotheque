...
// utilitaire : mettre une chaîne en minuscules (pour comparer les emails)
static void toLowerStr(char *s) {
    for (; *s; ++s)
        *s = (char)tolower((unsigned char)*s);
}

// Renvoie l'index d'un utilisateur à partir de son email (insensible à la casse), sinon -1
static int indexParEmail(Utilisateur *users, int nbUsers, const char *email) {
    char ref[100];
    strncpy(ref, email, sizeof(ref));
    ref[sizeof(ref)-1] = '\0';
    toLowerStr(ref);

    for (int i = 0; i < nbUsers; i++) {
        char cur[100];
        strncpy(cur, users[i].email, sizeof(cur));
        cur[sizeof(cur)-1] = '\0';
        toLowerStr(cur);
        if (strcmp(cur, ref) == 0)
            return i;
...
// Ajoute un nouvel utilisateur avec id auto-incrémenté et email unique
void ajouterUtilisateur(Utilisateur *users, int *nbUsers) {
    Utilisateur u;

    // ID auto-incrémenté : on prend le max existant + 1
    int nextId = 1;
    for (int i = 0; i < *nbUsers; i++)
    ...
    u.quota = 0; // Aucun livre emprunté au départ
...
void supprimerUtilisateur(Utilisateur *users, int *nbUsers, int id) {
    ...
    if (pos == -1) {
        printf("Utilisateur introuvable (id=%d).\n", id);
        return;
    }

    // Décale pour combler le trou
    for (int j = pos; j < *nbUsers - 1; j++)
        users[j] = users[j + 1];
...
// Stub simple d'authentification (à améliorer si besoin)
int authentifierUtilisateur(char *email) {
    // Retourne 1 si une chaîne non vide est fournie.
    // Plus tard, tu pourrais vérifier cet email dans le tableau d'utilisateurs.
